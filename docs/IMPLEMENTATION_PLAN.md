...
H2는 실제 MySQL primary/replica 같은 복제 구성을 제공하지 않는다. 그래서 로컬 H2 기본 설정에서는 master datasource와 slave datasource가 같은 DB URL을 바라보게 하고, `@Transactional(readOnly = true)`에 따라 라우팅되는 구조만 검증한다. 라우팅 테스트에서는 master/slave를 구분하기 위해 별도 H2 DB와 marker 값을 사용한다.
...
이번 과제에서는 Kafka를 직접 붙이지 않고 DB outbox와 publisher 상태 전이까지만 구현한다. 운영 환경에서는 scheduler, worker, CDC 기반 outbox relay가 Kafka 또는 RabbitMQ로 이벤트를 발행하는 구조를 고려했다고 README에 명시한다.
...
4. 같은 key가 있고 COMPLETED 상태이며 hash도 같으면 멱등하게 성공 처리한다.
...
2. Outbox publisher가 발행 가능한 PENDING 이벤트를 조회한다.
3. 로컬 구현에서는 외부 broker 없이 발행 성공을 시뮬레이션한다.
...
내 신청 목록 조회:
  path userId 기준으로 조회하며, 별도 X-USER-ID 검증은 하지 않음
...
- 결제 콜백으로 신청이 확정된다.
- 결제 금액이 다르면 실패한다.
- 같은 멱등성 key와 같은 요청은 중복 처리하지 않고 멱등하게 성공 처리한다.
