...
- `infrastructure`에는 outbox처럼 특정 도메인보다 인프라 성격이 강한 구현을 둔다.
...
```text
com.liveklass.infrastructure.outbox
  domain
    enums
  repository
  service
...
```text
com.liveklass.klass.command.repository
com.liveklass.klass.read.repository
com.liveklass.enrollment.command.repository
com.liveklass.enrollment.read.repository
```
...
```text
com.liveklass.klass.read.controller.response
com.liveklass.klass.command.controller.request
com.liveklass.klass.command.controller.response
```
